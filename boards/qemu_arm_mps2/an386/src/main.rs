...
static PANIC_RESOURCES: SingleThreadValue<
    PanicResources<mps2_base::ChipHw<cortexm4::CortexM4>, mps2_base::ProcessPrinterInUse>,
...
pub unsafe fn main() {
    ...
    let early = unsafe { mps2_base::early_init::<cortexm4::CortexM4>(&PANIC_RESOURCES) };

    // Create the actual chip instance (with specific Cortex-M variant)
    let chip = static_init!(
        mps2_base::ChipHw<cortexm4::CortexM4>,
        mps2_base::ChipHw::<cortexm4::CortexM4>::new(early.peripherals)
    );
